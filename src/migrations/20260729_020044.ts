...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`pages_rels\` (
  	...
  	FOREIGN KEY (\`diaries_id\`) REFERENCES \`diaries\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`pages_rels_diaries_id_idx\` ON \`pages_rels\` (\`diaries_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_pages_v_rels_diaries_id_idx\` ON \`_pages_v_rels\` (\`diaries_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`diary_media\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`alt\` text,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`diary_media_updated_at_idx\` ON \`diary_media\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`diary_media_created_at_idx\` ON \`diary_media\` (\`created_at\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`diary_media_filename_idx\` ON \`diary_media\` (\`filename\`);`)
  await db.run(sql`CREATE TABLE \`diaries_tags\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`tag\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`diaries\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`diaries_tags_order_idx\` ON \`diaries_tags\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`diaries_tags_parent_id_idx\` ON \`diaries_tags\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`diaries\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text,
  	\`content\` text,
  	\`excerpt\` text,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`diaries_entry_date_idx\` ON \`diaries\` (\`entry_date\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`diaries_slug_idx\` ON \`diaries\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`diaries_updated_at_idx\` ON \`diaries\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`diaries_created_at_idx\` ON \`diaries\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX \`diaries__status_idx\` ON \`diaries\` (\`_status\`);`)
  await db.run(sql`CREATE TABLE \`_diaries_v_version_tags\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`tag\` text,
  	\`_uuid\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`_diaries_v\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`_diaries_v_version_tags_order_idx\` ON \`_diaries_v_version_tags\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_diaries_v_version_tags_parent_id_idx\` ON \`_diaries_v_version_tags\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`_diaries_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`parent_id\` integer,
  	\`version_title\` text,
  	\`version_content\` text,
  	...
  	\`version_mood\` text,
  	\`version_weather\` text,
  	\`version_location\` text,
  	\`version_entry_date\` text,
  	\`version_generate_slug\` integer DEFAULT true,
  	\`version_slug\` text,
  	\`version_updated_at\` text,
  	\`version_created_at\` text,
  	...
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`latest\` integer,
  	FOREIGN KEY (\`parent_id\`) REFERENCES \`diaries\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`_diaries_v_parent_idx\` ON \`_diaries_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_diaries_v_version_version_entry_date_idx\` ON \`_diaries_v\` (\`version_entry_date\`);`)
  await db.run(sql`CREATE INDEX \`_diaries_v_version_version_slug_idx\` ON \`_diaries_v\` (\`version_slug\`);`)
  await db.run(sql`CREATE INDEX \`_diaries_v_version_version_updated_at_idx\` ON \`_diaries_v\` (\`version_updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_diaries_v_version_version_created_at_idx\` ON \`_diaries_v\` (\`version_created_at\`);`)
  await db.run(sql`CREATE INDEX \`_diaries_v_version_version__status_idx\` ON \`_diaries_v\` (\`version__status\`);`)
  await db.run(sql`CREATE INDEX \`_diaries_v_created_at_idx\` ON \`_diaries_v\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX \`_diaries_v_updated_at_idx\` ON \`_diaries_v\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_diaries_v_latest_idx\` ON \`_diaries_v\` (\`latest\`);`)
  ...
  await db.run(sql`CREATE INDEX \`search_rels_diaries_id_idx\` ON \`search_rels\` (\`diaries_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`diary_media_id\`) REFERENCES \`diary_media\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`diaries_id\`) REFERENCES \`diaries\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`categories_id\`) REFERENCES \`categories\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`users_id\`) REFERENCES \`users\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_diary_media_id_idx\` ON \`payload_locked_documents_rels\` (\`diary_media_id\`);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_diaries_id_idx\` ON \`payload_locked_documents_rels\` (\`diaries_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`header_rels_diaries_id_idx\` ON \`header_rels\` (\`diaries_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`footer_rels_diaries_id_idx\` ON \`footer_rels\` (\`diaries_id\`);`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`diary_media\`;`)
  await db.run(sql`DROP TABLE \`diaries_tags\`;`)
  await db.run(sql`DROP TABLE \`diaries\`;`)
  await db.run(sql`DROP TABLE \`_diaries_v_version_tags\`;`)
  await db.run(sql`DROP TABLE \`_diaries_v\`;`)
